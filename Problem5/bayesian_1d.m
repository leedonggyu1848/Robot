...
% 가우시안 분포 만들기
function rst = gaussian_distribution(x, mu, sigma)
	rst = (1 / (sigma * sqrt(2 * pi))) * exp(- (x - mu).^2 / (2 * sigma^2));
end

% measuremetn model 정의 -> p(z|x)
measurement_model = zeros(1, pos_size);
for i = 1:doors_size
	measurement_model = measurement_model + gaussian_distribution(1:pos_size, doors_pos(i), 1);
end
...
function rst = next_belief(prev_belief, measurement_model, sensored)
	...
	move_noise = gaussian_distribution(1:position_size, floor(position_size/2)+1+step, 0.5);
...
function plot_figure(doors_pos, true_pos, z_given_x, belief)
	...
	plot(true_pos, 1, 'bd', 'MarkerSize', 12, 'MarkerFaceColor', 'b'); % 실제 로봇 위치 표시
	xlim([0 pos_size]);
	ylim([0 2]);
	title('Real World');
	hold off;
